function [x, y, newParameters] = positionEstimator(test_data, modelParameters)
    ...
    % Post-processinng (strategy 2): adjust it based on prior knowledge
    % about expected behaviour. The mean value mean position 
    % for each time step and angle (x_med) and the predicted value by the
    % regressor (x_pred) have the same weight for the computation of the
    % final estimated position
    x_pred = 0.5*x_med+0.5*x_pred;
    y_pred = 0.5*y_med+0.5*y_pred;
